...
[pv_i, mliq_i, msld_i, Xm_i, X_i] = find_equilibrium(Tmelt, T_ref, T_i, DSC, MW, dHfus, dHvap, ...
...
ctot(1:nspec) = cgas.* DSC.vap_vol + sum(mliq_i) + sum(msld_i); %kg %Organic Liquid

% Total concentrations of the species 
corg_liq(1:nspec) = mliq_i;  %kg
corg_sld(1:nspec) = msld_i;  %kg
...
[tout, output] = ode45(@fluxes_bulk, time, input,  options, dt,nspec,...
    Cv_sld, Cp_liq, Cv_vap,...
    pstar,dHfus,dHvap,dHsub,dSfus,Tmelt,T_ref,MW,sigma1,rhol,rhos,...
    D_liq,Dn_air,mu1,alpha_m, ...
...
for ii = 1:length(time)
    [~, p_h2o(ii,:)] = fluxes_bulk(time(ii),output(ii,:)',dt,nspec,...
    Cv_sld, Cp_liq, Cv_vap,...
    pstar,dHfus,dHvap,dHsub,dSfus,Tmelt,T_ref,MW,sigma1,rhol,rhos,...
    D_liq,Dn_air,mu1,alpha_m, ...
    DSC, molec_group_flag,molec_group_stoich);
...
plot_2D({time', time',time'}, {press, h2o_partp, h2o_pvap},...
    ...
    plotDir,'DSC_Press',{'Total','P_{H2O}','P_{vap,H2O}'},lylog,1)
...
plot_2D({time',time',time',time'}, {Gc_out(:,nspec),OLc_out(:,nspec),...
    ...
    plotDir,'DSC_Equilib',{'Gas_{Dyn}','OrgLiq_{Dyn}','Gas_{Eq}','OrgLiq_{Eq}'},lylog,1)
